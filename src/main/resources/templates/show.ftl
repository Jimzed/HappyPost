...
<head>
    ...
    <link href="${sc}/bootstrap/css/bootstrap-responsive.min.css" rel="stylesheet" type="text/css"/>
    <!--[if lte IE 8]>
            ...
            <div class="navbar-inner">
                <div class="container-fluid">
                    <a class="brand" href="/"><i
                            class="icon-home icon-white"></i> Happy发帖<span class="sml_t"></span></a>
                    <ul class="nav user_menu pull-right">
                        ...
                        <li class="dropdown">
                            ...
                            <#if user??>
                                <img src="/user/pic/${user.userid}" alt="头像" class="user_avatar"/>
                                ${user.username}
                            <#else>
                            ...
                            <ul class="dropdown-menu">
                                <#if !user??>
                                    <li>
                                        <a data-toggle="modal" id="my" data-backdrop="static"
                                           href="#login" title="登录"
                                    ...
                                    <li>
                                        <a data-toggle="modal" id="my" data-backdrop="static"
                                           ...
                                           onclick="javascript:document.getElementById('submenu').innerHTML='注册'">注册
                                        </a>
                                    </li>
                                </#if>
                                <#if user??>
                                    <li>
                                        <a href="#post" title="灌水" data-toggle="modal"
                                           ...
                                           onclick="javascript:addz();document.getElementById('submenu').innerHTML='灌水'">发帖
                                        </a>
                                    </li>
                                </#if>
                                <#if user??>
                                <li class="divider"></li>
                                    <li>
                                        <a href="/user/logout"
                                           onclick="javascript:document.getElementById('submenu').innerHTML=''">退出当前用户
                                        </a>
                                    </li>
                                </#if>
                            </ul>
...
<!-- js cookie plugin -->
<script src="${sc}/js/jquery.cookie.min.js"></script>
<!-- main bootstrap js -->
...
<script type="text/javascript">
    ...
    $(function () {
        var username = $.cookie('jimkingu'); // 获得cookie
        var password = $.cookie('jimkingp');
    ...
    function add() {
        ...
        if ($("#postrootaction").val() === 'reply') {//回帖使用异步，否则不能定位到model
            $.ajax({
                type: "post",
                url: "/article/add",
                data: "title=" + $('#title').val() + "&content=" + txt + "&rootid=" + $("#postrootid").val() + "&userid=" + $('#userid').val() + "&id=" + $("#postrootid").val(),
                dataType: "text json",
                success: function (data) {
    ...
    */
    function showhuitie(id, uid, duid, data) {
        ...
        console.log("主贴id:" + id + "--浏览用户id:" + uid + "--主贴用户id：" + duid + "---");
        $(data.list).each(function (i) {
            ...
            if (i == 0) {
                //标记上第一个值能够被激活显示
                head = head + "<li class='active' ><a href='#tab_l" + i + "' data-toggle='tab'>" + data.list[i].title + "</a></li>";
            } else {
                head = head + "<li><a href='#tab_l" + i + "' data-toggle='tab'>" + data.list[i].title + "</a></li>";
            }
    ...
    function rshow(id, uid, duid) {
        ...
        $("#postrootid").val(id);
        //把该主贴用户id值付给post model的隐藏字段，一会添加从贴成功后走showhuitie有大用处
        $("#duid").val(duid);
        $.ajax({
            type: "post",
            url: "/article/queryreply",
            data: "id=" + id,
            dataType: "text json",
            success: function (data) {
